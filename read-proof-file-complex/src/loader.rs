...
pub fn load_json_public_input(json: &serde_json::Value) -> Vec<Fr> {
    ...
    let mut public_inputs = vec![str_to_fr(&elt[0])];
    public_inputs.push(str_to_fr(&elt[1]));
    public_inputs.push(str_to_fr(&elt[2]));
    public_inputs.push(str_to_fr(&elt[3]));
    public_inputs.push(str_to_fr(&elt[4]));
    public_inputs.push(str_to_fr(&elt[5]));
	public_inputs
}
